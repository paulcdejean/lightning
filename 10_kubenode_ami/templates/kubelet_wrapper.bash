...
ipv6_info=$(ip -json -br -6 addr show scope global)
ipv6_addr=$(jq -r '.[0]["addr_info"][0]["local"]' <<< "$ipv6_info")
...
# Configure cilium node
sed -i \
  -e s%__HOSTNAME__%$private_hostname% \
  -e s%__IPV4__%$ipv4_addr% \
  -e s%__IPV6__%$ipv6_addr% \
